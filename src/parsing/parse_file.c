...
static int	parse_texture_value(t_game *game, char *line)
{
	...
	if (parts == NULL || get_dtab_len(parts) != 2)
		...
		path = ft_strdup(parts[1]), game->texture[0].path = path, ret = 0;
	else if (ft_strncmp(parts[0], "SO", 3) == 0)
		path = ft_strdup(parts[1]), game->texture[1].path = path, ret = 0;
	else if (ft_strncmp(parts[0], "WE", 3) == 0)
		path = ft_strdup(parts[1]), game->texture[2].path = path, ret = 0;
	else if (ft_strncmp(parts[0], "EA", 3) == 0)
		path = ft_strdup(parts[1]), game->texture[3].path = path, ret = 0;
	else
		ret = 1;
	free_dtab(parts);
	return (ret);
...
static int	parse_color_parts(t_game *game, char *line, int is_floor)
{
	...
	while (index < 3)
	{
		value[index] = ft_atoi(rgb[index]);
		if (value[index] < 0 || value[index] > 255)
			return (free_dtab(parts), free_dtab(rgb), 1);
		index++;
	}
	if (is_floor)
		game->map.floor.r = value[0], game->map.floor.g = value[1], game->map.floor.b = value[2];
	else
		game->map.ceiling.r = value[0], game->map.ceiling.g = value[1], game->map.ceiling.b = value[2];
	free_dtab(parts);
	free_dtab(rgb);
	return (0);
}
